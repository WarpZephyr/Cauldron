...
public abstract class CauldronTestBase
{
    protected Cauldron CauldronInstance { get; private set; }

    protected ProjectManager ProjectManager => CauldronInstance.ProjectManager;

    protected CauldronTestBase()
    {
        CauldronInstance = new Cauldron(new VulkanGraphicsContext(), "Test", false);
        //CauldronInstance = new Cauldron(new OpenGLCompatGraphicsContext(), "Test", true);

        Cauldron.FirstFrame = false;
        Cauldron.FontRebuildRequest = false;

        var newProject = CreateERProject();

        CauldronInstance.ProjectManager.Projects.Add(newProject);
        CauldronInstance.ProjectManager.StartupProject(newProject);
    }

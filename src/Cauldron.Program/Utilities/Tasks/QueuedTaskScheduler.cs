...
public sealed class QueuedTaskScheduler : TaskScheduler, IDisposable
{
    ...
    private int DebugQueueCount
    {
        ...
        {
            var count = 0;
            foreach (KeyValuePair<int, QueueGroup> group in _queueGroups)
    ...
    private int DebugTaskCount =>
        (_targetScheduler != null ? _nonthreadsafeTaskQueue : (IEnumerable<Task>)_blockingTaskQueue)
        ...
        try
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        TaskLogs.AddLog($"[Cauldron] Task failed.", LogLevel.Error, LogPriority.High, ex);
                    }
    ...
    {
        // Create the queue
        QueuedTaskSchedulerQueue createdQueue = new(priority, this);
    ...
    private class QueuedTaskSchedulerDebugView
    {
        ...
        public IEnumerable<TaskScheduler> Queues
        {
            get
            {
                List<TaskScheduler> queues = new();
                foreach (KeyValuePair<int, QueueGroup> group in _scheduler._queueGroups)

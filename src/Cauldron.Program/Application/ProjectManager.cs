...
public class ProjectManager
{
    ...
    public ProjectManager(Cauldron baseEditor)
    {
    ...
    private void DisplayProjectActions(ProjectEntry curProject)
    {
        ...
        UIHelper.Tooltip("Set this project as the primary project to automatically load when Cauldron starts.");
        ...
        for (int i = 0; i < projectJsonList.Count; i++)
        {
            ...
            if (File.Exists(entry))
            {
                try
                {
                    ...
                    if (curProject == null)
                    {
                        TaskLogs.AddLog($"[Cauldron] Failed to load project: {entry}", LogLevel.Warning);
                    }
                    else
                    {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to load project: {entry}", LogLevel.Error,
                        LogPriority.High, e);
                }
            }
        }
    ...
    public void CreateLegacyProjectJSON(ProjectEntry curProject, bool firstTime = false)
    {
        ...
        if (firstTime)
        {
            if (File.Exists(jsonPath))
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to parse existing project.json: {e}", LogLevel.Error,
                        LogPriority.High, e);
                }
            }
        }

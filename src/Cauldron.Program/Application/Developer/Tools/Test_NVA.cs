...
public static class Test_NVA
{
    public static List<MismatchData> MismatchedNvas = new List<MismatchData>();
    ...
    public static void Display(Cauldron baseEditor, ProjectEntry project)
    {
        ...
        if (ImGui.Button("Check all NVAs for Byte-Perfect Match", DPI.StandardButtonSize))
        {
            Run(baseEditor);
        }
    ...
    public static bool Run(Cauldron baseEditor)
    {
        var curProject = baseEditor.ProjectManager.SelectedProject;

        MismatchedNvas = Test_MSB_Util.GetNvaMismatches(curProject);

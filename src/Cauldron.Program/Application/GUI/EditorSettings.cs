...
public class SystemTab
{
    ...
    public SystemTab(Cauldron baseEditor)
    {
    ...
    public void Display()
    {
        ...
        {
            ImGui.Checkbox("Check for new versions of Cauldron during startup",
                ref CFG.Current.System_Check_Program_Update);
            UIHelper.Tooltip("When enabled Cauldron will automatically check for new versions upon program start.");

            ImGui.Separator();

            UIHelper.WrappedText("By default, files are read by Cauldron in a strict manner. Data that is present in locations that it should not be will throw an exception.");

            UIHelper.WrappedText("This option will remove that strictness, and will cause Cauldron to ignore the invalid data when reading a file.");
        ...
        if (ImGui.CollapsingHeader("Project"))
        {
            ...
            UIHelper.Tooltip("Clear the project that has been set as primary, so no project is loaded on Cauldron start.");
        }
        ...
        if (ImGui.CollapsingHeader("Developer"))
        {
            ...
            UIHelper.Tooltip("Enables various tools meant for Cauldron developers only.");

            ImGui.Separator();

            if (ImGui.Button("Select##cauldronBuildDirSelect", DPI.StandardButtonSize))
            {
                var cauldronBuildDir = "";
                var result = PlatformUtils.Instance.OpenFolderDialog("Select Build directory", out cauldronBuildDir);

                if (result)
                {
                    CFG.Current.CauldronBuildFolder = cauldronBuildDir;
                }
            }

            ImGui.SameLine();

            ImGui.InputText("Cauldron Build Directory##cauldronBuildDir", ref CFG.Current.CauldronBuildFolder, 255);
            UIHelper.Tooltip("Select the build directory for Cauldron (where the Cauldron.sln is placed).");
        }
    }
...
public class MapEditorTab
{
    ...
    public MapEditorTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
    ...
    public void Display()
    {
        ...
        if (ImGui.CollapsingHeader("General", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            UIHelper.Tooltip("When enabled, a map's resources will be unloaded and released when a map is unloaded. If disabled, they are kept in memory until Cauldron closes.");
...
public class ModelEditorTab
{
    ...
    public ModelEditorTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
...
public class TextEditorTab
{
    ...
    public TextEditorTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
...
public class ParamEditorTab
{
    ...
    public ParamEditorTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
    ...
    public void Display()
    {
        if (BaseEditor.ProjectManager.SelectedProject != null)
        {
            ...
            if (ImGui.CollapsingHeader("Metadata", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                UIHelper.Tooltip("Use project-specific metadata instead of Cauldron's base versions.");
            }
            ...
            if (ImGui.CollapsingHeader("Regulation Data", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ...
                switch (curProject.ProjectType)
                {
                    ...
                    case ProjectType.ACVD:
                        ImGui.Checkbox("Restore stripped row names on load", ref CFG.Current.Param_RestoreStrippedRowNamesOnLoad_ACVD);
                        break;
            ...
            if (curProject.ParamData.ParamMemoryOffsets != null && curProject.ParamData.ParamMemoryOffsets.list != null)
            {
                ...
                if (ImGui.CollapsingHeader("Param Reloader", ImGuiTreeNodeFlags.DefaultOpen))
                {
                    ...
                    UIHelper.Tooltip("If enabled, the param reloader version will be set to the latest executable version whenever Cauldron is started.");
...
public class GparamEditorTab
{
    ...
    public GparamEditorTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
...
public class TextureViewerTab
{
    ...
    public TextureViewerTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
...
public class InterfaceTab
{
    ...
    public InterfaceTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    ...
    public void Display()
    {
        if (ImGui.CollapsingHeader("General", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            UIHelper.Tooltip("Adjusts the scale of the user interface throughout all of Cauldron.");
        ...
        if (ImGui.CollapsingHeader("Fonts", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            UIHelper.Tooltip("Adjusts the size of the font in Cauldron.");
            ...
            if (ImGui.Button("Set English font", DPI.StandardButtonSize))
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    Cauldron.FontRebuildRequest = true;
                }
            }
        ...
        {
            var storedDir = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Cauldron");
            ...
            if (File.Exists(storedPath))
            {
                ...
                if (ImGui.Button("Set##setImguiIni", DPI.StandardButtonSize))
                {
                    ...
                    PlatformUtils.Instance.MessageBox("Applied imgui.ini change. Restart Cauldron to apply changes.", "Information", MessageBoxButtons.OK);
                }
            }
...
public class ViewportTab
{
    ...
    public ViewportTab(Cauldron baseEditor)
    {
        BaseEditor = baseEditor;
    }
    ...
    public void Display()
    {
        ...
        if (ImGui.CollapsingHeader("Rendering", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            UIHelper.Tooltip("Enabling this option will allow Cauldron to render entities in the viewport.");

            // Toggle Texturing
            ImGui.Checkbox("Enable texturing", ref CFG.Current.Viewport_Enable_Texturing);
            UIHelper.Tooltip("Enabling this option will allow Cauldron to render the textures of models within the viewport.");

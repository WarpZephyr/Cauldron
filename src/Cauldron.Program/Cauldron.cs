...
public class Cauldron
{
    ...
    public unsafe Cauldron(IGraphicsContext context, string version, bool isLowRequirements)
    {
        LowRequirementsMode = isLowRequirements;

        _version = version;
        _programTitle = $"Cauldron - {_version}";
    ...
    /// <summary>
    /// Called when Cauldron is starting up
    /// </summary>
    private void Setup()
    {
        CFG.Setup();
    ...
    /// <summary>
    /// Called when Cauldron is shutting down
    /// </summary>
    private void Exit()
    {
        CFG.Save();
        ...
        if (!_initialLoadComplete)
        {
            ...
            catch (Exception e)
            {
                PlatformUtils.Instance.MessageBox($"Unable to save config during crash recovery.\n" +
                                                  $"If you continue to crash on startup, delete config in {Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Cauldron")}\n\n" +
    ...
    private unsafe void Update(float deltaseconds)
    {
        ...
        if (ImGui.BeginMainMenuBar())
        {
            ...
            if (ImGui.BeginMenu("Settings"))
            {
                ...
                UIHelper.Tooltip("Open the settings related to Cauldron's systems.");

                if (ImGui.MenuItem("Viewport"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.Viewport);
                }
                UIHelper.Tooltip("Open the settings related to Viewport in Cauldron.");

                if (ImGui.MenuItem("Interface"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.Interface);
                }
                UIHelper.Tooltip("Open the settings related to interface of Cauldron.");

                if (ImGui.MenuItem("Map Editor"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.MapEditor);
                }
                UIHelper.Tooltip("Open the settings related to Map Editor in Cauldron.");

                if (ImGui.MenuItem("Model Editor"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.ModelEditor);
                }
                UIHelper.Tooltip("Open the settings related to Model Editor in Cauldron.");

                if (ImGui.MenuItem("Param Editor"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.ParamEditor);
                }
                UIHelper.Tooltip("Open the settings related to Param Editor in Cauldron.");

                if (ImGui.MenuItem("Text Editor"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.TextEditor);
                }
                UIHelper.Tooltip("Open the settings related to Text Editor in Cauldron.");

                if (ImGui.MenuItem("Graphics Param Editor"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.GparamEditor);
                }
                UIHelper.Tooltip("Open the settings related to Gparam Editor in Cauldron.");

                if (ImGui.MenuItem("Texture Viewer"))
                {
                    Settings.ToggleWindow(SelectedSettingTab.TextureViewer);
                }
                UIHelper.Tooltip("Open the settings related to Texture Viewer in Cauldron.");

...
public class ResourceLocator
{
    ...
    public static string GetRelativePath(ProjectEntry project, string virtPath)
    {
        ...
        {
            TaskLogs.AddLog($"[Cauldron:DEBUG] Failed to find file in VFS: {relPath}", Microsoft.Extensions.Logging.LogLevel.Error);
        }
#endif
    ...
    /// <summary>
    /// Used to the external assets Cauldron loads in (i.e. the world map TPFs)
    /// </summary>
    ...
    public static string GetAbsolutePath(ProjectEntry project, string virtPath)
    {
        ...
        // --- CAULDRON ---
        // Used for designed TPF elements that are tend rendered as images within Cauldron
        // i.e. the world maps
        if (p[i].Equals("cauldron"))
        {
            i++;

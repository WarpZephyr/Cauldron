...
public class NVMNavmeshResource : IResource, IDisposable
{
    ...
    public bool _Load(string relativePath, AccessLevel al, string virtPath)
    {
        ...
        {
            TaskLogs.AddLog($"[Cauldron] Failed to load {relativePath} during NVMNavmeshResource load.", LogLevel.Error, LogPriority.High, e);
        }
    ...
    private unsafe void ProcessMesh(NVM mesh)
    {
        ...
        GeomBuffer =
            SceneRenderer.GeometryBufferAllocator.Allocate(vbuffersize, buffersize, (int)NavmeshLayout.SizeInBytes, 4);

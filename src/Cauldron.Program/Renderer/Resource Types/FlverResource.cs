...
public class FlverResource : IResource, IDisposable
{
    ...
    */
    ...
    public bool _Load(string relativePath, AccessLevel al, string virtPath)
    {
        ...
        {
            TaskLogs.AddLog($"[Cauldron] Failed to load {relativePath} during FlverResource load.", LogLevel.Error, LogPriority.High, e);
        }
        ...
        string mtd)
    {
        ...
        if (mpath == "")
        {
            ...
            if (material != null)
            {
                ...
                {
                    //ResourceLog.AddLog($"[Cauldron] {VirtPath}: Failed to find MTD string: {mtdstring} - {type}");
                    return;
                }

                path = tex.Path;
                //ResourceLog.AddLog($"[Cauldron] {VirtPath}: MTD: {path}");
            }
            ...
            if (curProject.ProjectType is ProjectType.ER or ProjectType.AC6 or ProjectType.NR)
            {
                ...
                if (matbin != null)
                {
                    ...
                    {
                        //ResourceLog.AddLog($"[Cauldron] {VirtPath}: Failed to find MATBIN string: {mtdstring} - {type}");
                        return;
                    }

                    path = tex.Path;
                    //ResourceLog.AddLog($"[Cauldron] {VirtPath}: MATBIN: {path}");
                }
            }
        }
        ...
        {
            //TaskLogs.AddLog($"[Cauldron] LISTENER for {virtualPath}");
        ...
        {
            // ResourceLog.AddLog($"[Cauldron:DEBUG] {VirtPath}: NO LISTENER for {textureVirtPath}", Microsoft.Extensions.Logging.LogLevel.Error);
        }
    }
    ...
    private bool LoadInternalDeS(AccessLevel al)
    {
        ...
        if (GPUMaterials.Any(e => e.GetNormalWBoneTransform()))
        {
            if (Bones != null)
            {
                StaticBoneBuffer = SceneRenderer.BoneBufferAllocator.Allocate(64 * (uint)Bones.Count, 64);
    ...
    private bool LoadInternalFast(BinaryReaderEx br)
    {
        ...
        if (GPUMeshes.Any(e => e.UseNormalWBoneTransform))
        {
            StaticBoneBuffer = SceneRenderer.BoneBufferAllocator.Allocate(64 * (uint)FBones.Count, 64);

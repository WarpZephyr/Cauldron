...
public class HavokCollisionResource : IResource, IDisposable
{
    ...
    public bool _Load(string relativePath, AccessLevel al, string virtPath)
    {
        ...
        else if (curProject.ProjectType is ProjectType.DS3)
        {
            ...
            {
                TaskLogs.AddLog($"[Cauldron] Failed to load {relativePath} during HavokCollisionResource load.", LogLevel.Error, LogPriority.High, e);
            }
        }
        // HKX - BB
        else if (curProject.ProjectType is ProjectType.BB)
            ...
            foreach (HKX.HKXObject obj in Hkx.DataSection.Objects)
            {
                ...
                if (obj is HKX.FSNPCustomParamCompressedMeshShape ncol)
                {
                    ...
                    foreach (HKX.HKXObject scene in Hkx.DataSection.Objects)
                    {
                        ...
                        {
                            TaskLogs.AddLog($"[Cauldron] Failed to load HKX.", LogLevel.Error, LogPriority.High, e);
                        }
                    }
                }
    ...
    private bool LoadInternal_DS3(AccessLevel al)
    {
        if (al == AccessLevel.AccessFull || al == AccessLevel.AccessGPUOptimizedOnly)
        {
            ...
            foreach (hknpBodyCinfo bodyInfo in physicsscene.m_systemDatas[0].m_bodyCinfos)
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to load HKX2.", LogLevel.Error, LogPriority.High, e);
                }
            }
        ...
        if (al == AccessLevel.AccessFull || al == AccessLevel.AccessGPUOptimizedOnly)
        {
            ...
            if (scene is HKLib.hk2018.hknpPhysicsSceneData physicsscene)
            {
                foreach (HKLib.hk2018.hknpBodyCinfo bodyInfo in physicsscene.m_systemDatas[0].m_bodyCinfos)
                {
                    ...
                    {
                        TaskLogs.AddLog($"[Cauldron] Failed to load HKLIB.", LogLevel.Error, LogPriority.High, e);

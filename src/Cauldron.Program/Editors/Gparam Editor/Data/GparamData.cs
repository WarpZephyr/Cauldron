...
public class GparamData
{
    ...
    public GparamData(Cauldron baseEditor, ProjectEntry project)
    {
    ...
    public async Task<bool> SetupGparamInfo()
    {
        ...
        var projectFolder = Path.Join(Project.ProjectPath, ".cauldron", "Assets", "GPARAM", ProjectUtils.GetGameDirectory(Project.ProjectType));
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the GPARAM information: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the GPARAM information: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
    ...
    public async Task<bool> SetupGparamEnums()
    {
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the GPARAM enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the GPARAM enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }

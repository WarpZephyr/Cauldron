...
public class ModelData
{
    ...
    public ModelData(Cauldron baseEditor, ProjectEntry project)
    {
    ...
    public async Task<bool> SetupFlverInfo()
    {
        ...
        var projectFolder = Path.Join(Project.ProjectPath, ".cauldron", "Assets", "FLVER");
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the FLVER information: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the FLVER information: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the FLVER enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the FLVER enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }

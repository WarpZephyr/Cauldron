    ...
    public void ExtractMainFile()
    {
        ...
        try
        {
            ...
            if (!File.Exists(absPath))
            {
                ...
                TaskLogs.AddLog($"[Cauldron:File Browser] Extracted {absPath}");
        ...
        {
            TaskLogs.AddLog($"[Cauldron:File Browser] Failed to write file: {fileEntry.Path}", LogLevel.Error, LogPriority.High, e);
        }
    }
    ...
    public void ExtractInternalFile()
    {
        ...
        if (binderType is ResourceContainerType.BND)
        {
            if (Project.ProjectType is ProjectType.ACFA or ProjectType.ACV or ProjectType.ACVD)
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron:File Browser] Failed to read {targetFile.Path}.", LogLevel.Error, LogPriority.High, e);
                }
            }
            else
            {
        ...
        if(extractData.Length > 0)
        {
            ...
            TaskLogs.AddLog($"[Cauldron:File Browser] Extracted {filename}");
        }
    }
}

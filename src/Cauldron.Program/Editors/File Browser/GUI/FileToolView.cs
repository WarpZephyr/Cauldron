...
public class FileToolView
{
            ...
            if (ImGui.Button("Delete Unpacked Data", DPI.WholeWidthButton(windowWidth, 24)))
            {
                ...
                foreach (var entry in TopFolderList)
                {
                    ...
                    if (Directory.Exists(absFolder))
                    {
                        ...
                        {
                            TaskLogs.AddLog($"[Cauldron] Failed to delete folder: {absFolder}", LogLevel.Error, LogPriority.High, e);
                        }
                    }
                }
            }
    ...
    public async Task UnpackGameAsync(FileDictionary targetFileDictionary)
    {
        ...
        foreach (var entry in targetFileDictionary.Entries)
        {
            tasks.Add(Task.Run(async () =>
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to write file: {entry.Path}", LogLevel.Error, LogPriority.High);
        ...
        {
            TaskLogs.AddLog("[Cauldron] Unpacking was cancelled.", LogLevel.Warning);
        }
    ...
    public async Task DeleteUnpackedDataAsync()
    {
        ...
        {
            TaskLogs.AddLog("[Cauldron] Deleting was cancelled.", LogLevel.Warning);
        }

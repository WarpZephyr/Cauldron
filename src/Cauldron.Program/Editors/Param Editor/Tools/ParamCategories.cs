...
public static class ParamCategories
{
    ...
    public static void Display(ParamEditorScreen editor)
    {
        ...
        UIHelper.Tooltip("Permanently save the current param categories to your project's .cauldron folder, so they persist across sessions.");
    ...
    public static void RestoreDefault(ParamEditorScreen editor)
    {
        ...
        if (File.Exists(sourceFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog("[Cauldron] Failed to deserialize param categories", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog("[Cauldron] Failed to read param categories", LogLevel.Error, LogPriority.High, e);
            }
        }
        else
        {
            TaskLogs.AddLog("[Cauldron] Failed to find default param categories for game", LogLevel.Error, LogPriority.High);
        }
    }
    ...
    public static void Write(ParamEditorScreen editor)
    {
        ...
        var modResourceDir = Path.Join(editor.Project.ProjectPath, ".cauldron", "Assets", "PARAM", ProjectUtils.GetGameDirectory(editor.Project));
        ...
        {
            TaskLogs.AddLog("[Cauldron] Failed to write project param categories", LogLevel.Error, LogPriority.High, ex);
        }

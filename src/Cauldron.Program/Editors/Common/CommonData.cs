...
public class CommonData
{
    ...
    public CommonData(Cauldron baseEditor, ProjectEntry project)
    {
    ...
    public async Task<bool> SetupAliases()
    {
        ...
        [
            ...
            Path.Join(Project.ProjectPath,".cauldron","Assets","Aliases")
        ];
        ...
        foreach (string sourceFile in sourceFiles)
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the aliases: {sourceFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the aliases: {sourceFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
    ...
    public async Task<bool> SetupProjectEnums()
    {
        ...
        var projectFolder = Path.Join(Project.ProjectPath, ".cauldron", "Project");
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the Project Enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the Project Enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }

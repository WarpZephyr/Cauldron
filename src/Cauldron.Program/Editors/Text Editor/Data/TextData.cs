...
public class TextData
{
    ...
    public TextData(Cauldron baseEditor, ProjectEntry project)
    {
    ...
    public FileDictionary SetupFmgFilelist()
    {
        ...
        {
            fmgDictionary.Entries = Project.FileDictionary.Entries
                .Where(e => (e.Folder.StartsWith("/lang") || e.Folder.StartsWith("/region")))
                .Where(e => e.Extension == "fmg").ToList();
        }
    ...
    public LanguageDef SetupLanguageDef()
    {
        ...
        if (File.Exists(file))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the FMG Language Def: {file}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the FMG Language Def: {file}", LogLevel.Error, LogPriority.High, e);
            }
        }
    ...
    public ContainerDef SetupContainerDef()
    {
        ...
        if (File.Exists(file))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the FMG Container Def: {file}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the FMG Container Def: {file}", LogLevel.Error, LogPriority.High, e);
            }
        }
    ...
    public AssociationDef SetupAssociationDef()
    {
        ...
        if (File.Exists(file))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the FMG Association Def: {file}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the FMG Association Def: {file}", LogLevel.Error, LogPriority.High, e);
            }
        }

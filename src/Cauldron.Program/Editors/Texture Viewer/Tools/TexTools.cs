    ...
    public void ExportTextureHandler()
    {
        ...
        if (currentTexture != null)
        {
            ...
            if (exportPath != "")
            {
                ...
                {
                    var result = PlatformUtils.Instance.MessageBox($"Overwrite existing file at {exportFilePath}?", $"Cauldron", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                ...
                {
                    write = false;
                    PlatformUtils.Instance.MessageBox($"Directory is not valid.", $"Cauldron", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                ...
                if (write)
                {
                    ...
                    {
                        PlatformUtils.Instance.MessageBox($"{filename} exported.", $"Cauldron", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            else
            {
                PlatformUtils.Instance.MessageBox($"Export Destination is not set.", $"Cauldron", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        else
        {
            PlatformUtils.Instance.MessageBox($"No texture is currently being viewed.", $"Cauldron", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    }

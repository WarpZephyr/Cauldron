...
public class MaterialData
{
    ...
    public MaterialData(Cauldron baseEditor, ProjectEntry project)
    {
    ...
    public async Task<bool> SetupMaterialDisplayConfiguration()
    {
        ...
        var projectFolder = Path.Join(Project.ProjectPath, ".cauldron", "Assets", "MATERIAL", ProjectUtils.GetGameDirectory(Project.ProjectType));
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the Material Display Configuration: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the Material Display Configuration: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }

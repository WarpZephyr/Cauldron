...
public class MapData
{
    ...
    public MapData(Cauldron baseEditor, ProjectEntry project)
    {
    ...
    public async Task<bool> SetupMapObjectNames()
    {
        ...
        var projDir = Path.Combine(Project.ProjectPath, ".cauldron", "Project", "Community Map Object Names");
    ...
    public async Task<bool> SetupMapObjectSelections()
    {
        ...
        if (File.Exists(projectFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the Map Object Selections: {projectFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the Map Object Selections: {projectFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
        else
        {
    ...
    public async Task<bool> SetupMsbInfo()
    {
        ...
        var projectFolder = Path.Join(Project.ProjectPath, ".cauldron", "Assets", "MSB", ProjectUtils.GetGameDirectory(Project.ProjectType));
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the MSB information: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the MSB information: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the MSB enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the MSB enums: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the MSB masks: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the MSB masks: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
    ...
    public async Task<bool> SetupMapSpawnStates()
    {
        ...
        if (File.Exists(targetFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron] Failed to deserialize the Map Spawn States: {targetFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron] Failed to read the Map Spawn States: {targetFile}", LogLevel.Error, LogPriority.High, e);
            }
        }

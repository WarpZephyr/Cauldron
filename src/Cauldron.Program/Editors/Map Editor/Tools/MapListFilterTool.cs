...
public class MapListFilterTool
{
    ...
    {
        var filterFolder = Path.Join(Project.ProjectPath, ".cauldron", "MSB");
        ...
        if (File.Exists(filterFile))
        {
            try
            {
                ...
                {
                    TaskLogs.AddLog($"[Cauldron:{Project.ProjectName}:Map Editor] Failed to deserialize the Quick Filter List Collection: {filterFile}", LogLevel.Error, LogPriority.High, e);
                }
            }
            catch (Exception e)
            {
                TaskLogs.AddLog($"[Cauldron:{Project.ProjectName}:Map Editor] Failed to read the Quick Filter List Collection: {filterFile}", LogLevel.Error, LogPriority.High, e);
            }
        }
        else
        {

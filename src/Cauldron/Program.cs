...
public static class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        {
            Instance = new Cauldron(new OpenGLCompatGraphicsContext(), _version, IsLowRequirements);
        }
        else
        {
            Instance = new Cauldron(new VulkanGraphicsContext(), _version, IsLowRequirements);
        }
    ...
    static void ExportCrashLog(List<string> exceptionInfo)
    {
        ...
        exceptionInfo.Insert(0, $"Cauldron - Version {_version}\n");
        ...
        if (exceptionInfo.Count > 10)
            PlatformUtils.Instance.MessageBox($"Cauldron has run into an issue.\nCrash log has been generated at \"{crashLogPath}\".",
                $"Cauldron Unhandled Error - {_version}", MessageBoxButtons.OK, MessageBoxIcon.Error);
        else
            PlatformUtils.Instance.MessageBox($"Cauldron has run into an issue.\nCrash log has been generated at \"{crashLogPath}\".\n\nCrash Log:\n{string.Join("\n", exceptionInfo)}",
                $"Cauldron Unhandled Error - {_version}", MessageBoxButtons.OK, MessageBoxIcon.Error);
    }
